...
BM25_CORPUS_PATH = (
    Path(__file__).parent.parent / "data" / "indexes" / "bm25_corpus.json"
)
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    if EMBEDDING_MODEL.casefold() == "bm25":
        raise ValueError(
            "BM25 is a lexical retriever, not a SentenceTransformer model. "
            "Use src.task6_lexical_search.lexical_search()."
        )
...
def save_bm25_corpus(chunks: list[dict]) -> None:
    """Lưu chunks để Task 6 tạo BM25 index mà không cần tải model."""
...
def run_pipeline() -> None:
    ...
    if EMBEDDING_MODEL.casefold() == "bm25":
        ...
        print(f"Corpus: {BM25_CORPUS_PATH}")
        return
